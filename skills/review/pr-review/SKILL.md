...
프로젝트에 설정 파일(`rules/project-params.local.md`)이 있으면 다음 설정을 읽는다:
- `base_branch`: PR base 브랜치 (기본값: `gh repo view --json defaultBranchRef`로 자동 감지)
- `fork_workflow`: fork 기반 워크플로우 여부 (기본값: false)
  - `true`: origin=fork, upstream=org repo 구조
  - `false` 또는 미설정: 단일 remote (upstream)
...
# PR remote 판별
# rules/project-params.local.md의 fork_workflow 설정 참조
# 기본 remote는 항상 upstream
REMOTE="upstream"
