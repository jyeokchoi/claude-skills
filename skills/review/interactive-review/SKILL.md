...
체크되지 않은 다음 파일(`{next-file}`)에 대해 **요약 → 사용자 확인 → 리뷰** 순으로 진행한다.
...
#### 4-B. 파일 요약 (explore subagent)

`explore` subagent에게 파일 목적·구조 요약을 위임한다:
...
### 파일 목적
한 문장으로 이 파일이 하는 일을 설명하라.

### 주요 구성 요소
- 핵심 함수/클래스/타입 목록 (각 1줄 설명)

### 의존 관계
- 이 파일이 의존하는 주요 모듈/인터페이스
- 이 파일을 사용할 것으로 예상되는 컨텍스트

### 설계 의도 (추정)
저자가 이 파일을 통해 해결하려 한 문제가 무엇인지 추정하라.

IMPORTANT: Do NOT use Bash, Glob, or Grep. 제공된 파일 내용만으로 분석하라.
"""
)
```

요약 결과를 `{file_summary}`로 저장한다.

**사용자에게 요약 출력:**
```
📄 {next-file} ({checked+1}/{total})
...
해당 파일 체크리스트 항목 아래에 요약을 추가한다:
```markdown
- [ ] src/auth/login.ts — 인증 진입점
  > 목적: JWT 토큰 기반 로그인 처리
  > 주요 구성: loginUser(), refreshToken(), validateCredentials()
```

#### 4-C. 코드 리뷰 (code-reviewer subagent)

`code-reviewer` subagent에게 파일 내용과 요약을 함께 전달한다.
요약은 참고 컨텍스트일 뿐이며, 반드시 코드 자체를 기준으로 비판적으로 판단하도록 명시한다:
...
## 사전 분석 요약 (참고용 — 비판적으로 검토할 것)

{file_summary}

위 요약은 자동 생성된 것으로 **틀릴 수 있다**. 요약을 맹신하지 말고, 반드시 파일 내용을 직접 읽어 독립적으로 판단하라.
특히 다음에 주의하라:
- 요약이 설계 의도를 과도하게 긍정적으로 해석했을 가능성
- 요약이 놓친 엣지 케이스나 숨겨진 가정
- "의도적 트레이드오프"로 포장된 실제 문제
...
### 요약과의 불일치
요약이 잘못 파악했거나 과도하게 긍정적으로 해석한 부분이 있으면 명시하라.
없으면 "없음"으로 표기.
