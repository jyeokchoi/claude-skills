...
`/explain-code`를 실행하여 전체 변경사항의 하이레벨 요약과 리뷰 순서를 얻는다.

`explain-code`의 출력 전체(하이레벨 요약, 아키텍처, 주요 변경 파일, 추천 리뷰 순서)를 파싱하여 state 파일을 생성한다:
...
- **`explain-code` 전체 요약**: 하이레벨 요약, 아키텍처 다이어그램, 주요 변경 파일 설명
- 체크리스트: `- [ ] {파일경로} — {리뷰 순서 이유}`
...
## Codebase 요약 (explain-code)

{explain-code 전체 출력 — 하이레벨 요약, 아키텍처, 주요 변경 파일}
...
체크되지 않은 다음 파일(`{next-file}`)을 오케스트레이터가 직접 읽어 내용을 수집한 뒤, `code-reviewer` subagent에게 위임한다. state 파일에 저장된 `explain-code` 전체 요약을 컨텍스트로 함께 전달한다.

**오케스트레이터가 파일 내용 수집:**
```
file_content = Read("{next-file}")
codebase_summary = state 파일의 "Codebase 요약" 섹션
```
...
## Codebase 전체 요약 (참고용 — 비판적으로 검토할 것)
...
- 코드베이스 전체 맥락이 이 파일의 문제를 정당화하지 않는 경우
...
### 요약과의 불일치
codebase 요약이 잘못 파악했거나 과도하게 긍정적으로 해석한 부분이 있으면 명시하라.
없으면 "없음"으로 표기.
