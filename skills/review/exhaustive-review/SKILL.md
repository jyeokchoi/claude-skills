...
- Codex 호출은 중재자가 수행하며, 타임아웃 또는 실패 시 해당 라운드 검증만 생략하고 토론을 계속한다
</Execution_Policy>
...
3. Codex CLI 가용성 확인:
   ```
   Bash(command="which codex 2>/dev/null && codex --version 2>/dev/null")
   ```
   - 성공 → `CODEX_AVAILABLE=true`, 실패 → `CODEX_AVAILABLE=false`
   - 세션 내 1회 캐싱
...
2. 중재자 생성 (백그라운드):
   ...
   Task(
     ...
     prompt="{MODERATOR_PROMPT with REVIEW_CONTEXT and codex_available={CODEX_AVAILABLE} embedded}"
   )
   ```
...
### Step 3.5: Codex Validation (codex_available=true인 경우에만)

IF codex_available = false → 이 스텝 전체 생략 (silent skip).
...
6. 다음 라운드 메시지에 Codex 출력 추가:
   "## External Validation (Codex) — Round {N}
    {codex_output}
    Consider Codex's fact-checks when formulating your response."

패스트트랙 조건:
- 토론 0라운드 (전체 패스트트랙): Codex 호출 없음
- 1라운드 패스트트랙: 초기 리뷰 후 Codex 1회 → 보고서에 포함
- 일반 토론: 매 라운드 후 Codex 호출
...
2. Send to all three reviewers:
   ...
   - Codex 검증이 수행된 경우: Codex의 fact-check와 독립 발견을 포함
...
## External Validation (Codex)
{CODEX_AVAILABLE=true이고 Codex 검증이 1회 이상 수행된 경우에만 표시}

### Summary
- Rounds validated: {list}
- Independent findings: {count}

### Round-by-Round Contributions
#### Round {N}
- Fact-Checks: {contested items 검증}
- Independent Findings: {새로 발견한 이슈}
...
#### External Validator (Codex)
{Codex 검증이 수행된 라운드에만 포함}
- Fact-checks on contested findings
...
```
You are an independent external validator for a code review debate.

## Your Role
- Fact-check reviewers' positions against the actual code
- Identify findings all reviewers may have missed
- Provide independent severity assessments for contested items

## Code Context
{REVIEW_CONTEXT}

## Current Reviewer Positions (Round {N})
### Advocate
{findings + verdict}
### Surgeon
{findings + verdict}
...
### Contested Items
{list}
{IF round >= 2: ### Previous Codex Output (Round {N-1}) ...}
...
For each contested finding:
- **Re: {title}**
  - Most accurate position: {Advocate|Surgeon|Challenger}
  - Evidence: {file:line + reasoning}
  - Your severity: {CRITICAL|HIGH|MEDIUM|LOW}
...
- Codex 프롬프트는 contested items 관련 파일 위주로 구성하며, 이전 Codex 출력은 최신 1개만 포함한다
</Context_Management>

<Escalation_And_Stop_Conditions>
- **최대 7라운드** — 합의 또는 안정화될 때까지 토론 진행. 7라운드 이후에도 합의 안 되면 각 페르소나의 최종 입장을 병기하여 보고서 작성
- 입장이 안정화되면 중단 (연속 라운드 간 변화 없음)
- **패스트트랙:** 초기 리뷰 후 세 명 모두 APPROVE + CRITICAL/HIGH 없음 → 토론 생략, 즉시 보고서 작성. Codex 호출 없음
- **1라운드 패스트트랙:** 세 명 모두 APPROVE + CRITICAL 없음 + HIGH 1~2개 → 1라운드 실행 후 보고서 작성. 초기 리뷰 후 Codex 1회 실행하여 보고서에 포함
- CRITICAL 발견이 있으면 반드시 토론해야 함 — 있을 경우 생략 불가
- **일반 토론:** 매 라운드 종료 후 Codex 호출 (CODEX_AVAILABLE=true인 경우)
