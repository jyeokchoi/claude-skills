...
| 설정 | 기본값 | 비고 | 참조 스킬 |
|------|--------|------|-----------|
| `test_command` | (자동 탐지: package.json scripts) | 테스트 실행 명령 (e.g., `yarn test --run`) | vimpl, vtest, vqa, vanalyze |
| `test_framework` | (자동 탐지) | 테스트 프레임워크 (e.g., `vitest`, `jest`, `pytest`, `go test`) | vtest |
...
| 설정 | 기본값 | 비고 | 참조 스킬 |
|------|--------|------|-----------|
| `timezone` | (시스템 시간대) | 타임스탬프 시간대 (e.g., `Asia/Seoul`, `America/New_York`) | worklog-start, _shared/update-worklog, vanalyze |
| `timestamp_suffix` | (없음) | 타임스탬프 뒤에 붙는 문자열 (e.g., `KST`) | _shared/update-worklog, vanalyze |

## Project Context

| 설정 | 기본값 | 비고 | 참조 스킬 |
|------|--------|------|-----------|
| `project_type` | (자동 탐지) | `frontend` / `backend` / `fullstack` / `cli` / `library` | vplan, vtest, vanalyze |
| `shared_types_dir` | (없음) | 공유 타입 디렉토리 (e.g., `typings/`). 코드 분석 시 참조 | vplan, vanalyze |
| `completion_promise` | `**WORKLOG_TASK_COMPLETE**` | ralph loop 완료 감지용 (OMC 내부) | vimpl, vwork |
| `slack_integration` | `false` | `true`면 worklog-start에서 Slack 컨텍스트 수집 | worklog-start |
| `gist_visibility` | `secret` | Gist 공개 설정 (`secret` / `public`) | worklog-finish |

## Skill Routing (선택)

워크플로우 스킬 간 참조 시 사용할 이름:
- 기본: `/vwork`, `/vplan`, `/vimpl`, `/vqa`, `/vtest`, `/vanalyze`, `/worklog-start`, `/worklog-finish` 등
