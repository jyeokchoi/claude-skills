...
| 설정 | 기본값 | 용도 |
|------|--------|------|
| `base_branch` | (자동 탐지: `gh repo view --json defaultBranchRef`) | git merge/checkout 기준 브랜치 |
| `test_command` | (프로젝트에서 탐지) | 테스트 실행 명령 |

설정이 없으면 자동 탐지, 그래도 없으면 사용자에게 질문 → `project_memory_add_note`로 기억.
...
Run automated scans?
  1. Yes, run security audit (npm audit / yarn audit / pip audit 등 프로젝트에 맞는 도구) + recommended scans
...
3. When task complete:
   ...
   - Merge to base branch: git checkout {base_branch} && git merge task-{id}-{slug}
