...
프로젝트 설정 파일(`rules/project-params.local.md`)이 있으면 다음 설정을 읽는다:
...
**`slack-write` 스킬을 로드하여 Slack 포맷 규칙과 전송 워크플로우를 따른다.**
- `slack-write`의 포맷 규칙(볼드, 불릿, 링크, 이모지, 서명 등)을 모두 적용한다.
- `slack-write`의 사용 금지 항목(마크다운 테이블, `#` 헤딩, `[text](url)` 링크)을 준수한다.
...
## 4단계~5단계: 사용자 검토 및 Slack 게시

**`slack-write` 스킬의 워크플로우를 그대로 따른다:**
1. 초안을 사용자에게 보여주고 승인받는다.
2. 채널 ID를 Slack MCP 도구로 확인한다.
3. `content_type: "text/plain"`으로 전송한다.
4. 스레드 댓글인 경우 `thread_ts`를 설정한다 (파싱된 URL에서 추출).
5. `slack-write`의 실수 방지 체크리스트를 모두 통과하는지 확인한다.
