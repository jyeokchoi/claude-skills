...
모드 결정 후 `_shared/ralph-activation.md`를 로드하고 해당 절차를 따른다.
...
`codex_available={codex_available}` 파라미터를 추가한다. `cli_type=codex`인 경우 `cli_type={cli_type}`도 추가한다.
