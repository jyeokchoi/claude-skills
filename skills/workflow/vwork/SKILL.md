...
argument-hint: 'Usage: /vwork [worklog-path | task-name | ISSUE-KEY] [brief]'
# human-in-the-loop 전용 오케스트레이터.
---
...
`_shared/state-preservation-rule.md`를 로드하고 해당 규칙을 따른다. vwork 추가 규칙:
...
- 스폰 시 append, 정리(shutdown_request 응답 수신 또는 `mcp__team__omc_run_team_cleanup` 완료) 시 제거.
...
- 형식: `{"implementer": {"cli_type": "codex", "team_name": "..."}, "tester": {"cli_type": "gemini", "team_name": "..."}}`
...
- **반드시 `Skill("worklog-start", ...)`로 워크로그를 생성한다.** 워크로그를 수동으로 생성하지 않는다 (Write/Bash 금지).
...
- 미발견 시: 사용자에게 요청, **반드시 `Skill("worklog-start", ...)`로 생성** (수동 생성 금지)
...
vwork는 항상 세션 기반 human-in-the-loop로 동작한다. ralph loop이 상시 활성화되어 컴팩션 복원력을 보장하고, 모든 작업은 팀원에게 위임된다.

- 각 phase 완료 후 사용자에게 다음 단계 진행 여부를 확인한다.

**ralph loop 활성화:**

`_shared/ralph-activation.md`를 로드하고 해당 절차를 따른다. 완료 후 **멈추지 않고** 즉시 Step 3을 실행한다.
...
`_shared/resolve-project-type.md`를 로드하고 해당 절차를 따른다. vwork는 항상 독립 실행 경로를 사용한다 (오케스트레이터이므로).

**CLI 가용성 확인:**
`_shared/cli-runtime-check.md` 섹션 2를 참조한다:
1. `Bash(command="which codex 2>/dev/null && codex --version 2>/dev/null")` → 성공 시 `codex_available = true`, 실패/오류 시 `false`
2. `Bash(command="which gemini 2>/dev/null && gemini --version 2>/dev/null")` → 성공 시 `gemini_available = true`, 실패/오류 시 `false`
...
**상태 기록 완료 후 멈추지 않고 즉시 Priority notepad 기록 → Step 3-B를 연속 실행한다.**

- **Priority notepad 초기 기록** (Notepad 라이프사이클 참조):
  ```
  <remember priority>vwork활성|mode:step|phase:{phase}|team:미정
  wl:{WORKLOG_DIR}
...
**팀 생성 완료 후 멈추지 않고 즉시 Step 3-C → Step 4를 연속 실행한다.**
...
3. 아래 phase 전이 질문 테이블에 따라 AskUserQuestion으로 확인
4. 사용자 선택이 "진행"으로 확정된 경우에만 다음 phase로 업데이트
...
|------|------|------|
| ANALYZE → PRD | "분석이 완료되었습니다. PRD로 진행할까요?" | "PRD 수정" |
| PRD → PLAN | "PRD가 완료되었습니다. 플래닝으로 진행할까요?" | "PRD 수정" |
...
- **codex 크로스 검증은 vqa 내부에서 처리한다** (Agent 5). vwork에서 별도 codex CLI 워커를 추가 스폰하지 않는다. `codex_available` 값은 vwork state를 통해 vqa에 전달되며, vqa가 Agent 5 실행 여부를 자체 판단한다.
...
- 누락/다중/미지 값이면: **phase 전이 금지** → 워크로그 에러 기록 → 사용자에게 질문
...
**가장 먼저 `Skill("worklog-finish", "{WORKLOG_DIR}")`을 호출한다.** 이 스킬이 다음을 모두 처리한다:
- Jira 이슈 업데이트
- Worklog를 Gist에 보관 및 로컬 삭제
- PR 생성 (사용자 확인)
- Worktree 정리 (사용자 확인)

**오케스트레이터가 commit, push, PR 생성, Jira 업데이트, Gist 업로드, worktree 삭제를 직접 수행하는 것은 금지된다.** 이 작업들은 모두 `/worklog-finish`의 책임이다.

`/worklog-finish` 완료 후:
- `spawned_agents` 전원 정리 (`cli_workers` 맵 참조하여 유형별 분기):
  - `cli_workers[{팀원명}]`이 존재하면 → `mcp__team__omc_run_team_cleanup(job_id="{cli_workers[{팀원명}].team_name}")`
...
- **초기화 중 멈추지 않는다.** Step 1~4는 연속 실행이다. 중간에 사용자에게 보고하거나 일시정지하지 않는다. state_write/notepad 결과를 받더라도 즉시 다음 단계로 진행한다.
- **워크로그가 유일한 출처.** 모든 phase는 워크로그에서 읽고 워크로그에 쓴다.
- **phase 필드를 항상 최신으로 유지.** 각 phase 전후에 반드시 업데이트.
- **phase를 건너뛰지 않는다.** 단, 신규 기능은 PRD부터 시작하므로 ANALYZE를 건너뛰는 것이 아니라 시작점이 다른 것이다.
- **ralph loop을 활성화한다.** 컨텍스트 윈도우 한계를 넘어 지속성을 보장.
- **phase 전이 시 항상 질문한다.** 사용자 확인 없이 자동 진행하지 않는다.
- **피드백 루프는 상한이 있다.** 루프당 최대 5회 반복. 각 반복을 Decisions에 기록한다.
- **오케스트레이터는 실행하지 않는다.** 모든 작업은 반드시 팀원에게 `SendMessage`로 위임한다. `Read`/`Edit`/`Write`/`Bash`로 직접 코드를 읽거나 수정하는 것은 **금지**. 예외: 워크로그 읽기, state/notepad 관리, Step 0 SKILL.md 읽기, VERIFY verdict 파싱을 위한 `{WORKLOG_DIR}/report.md` 읽기.
...
- **워크플로우 순서 강제.** 모든 요청을 현재 phase 상태 머신에 맞게 해석한다.
- **사용자 명시 요청 없이는 페이즈 생략/워크플로우 변경 금지.** 오케스트레이터가 임의로 phase를 건너뛰거나 순서를 재배치하거나 대체 플로우로 전환하지 않는다.
