...
```
mcp__team__omc_run_team_start(count=1, provider="gemini", task="테스트 작성: {gap description}: 위 codex 경로와 동일한 프롬프트")
# 출력: {"jobId": "{job_id}", ...}
mcp__team__omc_run_team_status(team_name="{WORKLOG_SLUG}-test-write")
```
