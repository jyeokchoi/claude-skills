...
아래 표가 vplan의 단계 전이 단일 진실 기준이다.

| 현재 단계 | 완료 조건 | 다음 단계 | 주요 출력 |
|-----------|-----------|----------|----------|
| 사전 준비 | 컨텍스트/BASE_REF/project_type 확정 | Stage 1 또는 Stage 2 | 준비 컨텍스트 |
| Stage 1 (PRD) | PRD 작성 + 검증 + 사용자 검토 완료 | Stage 2 또는 종료 | `prd.md` |
| Stage 2 (동작/인터페이스) | 동작 명세 확정 | Stage 3 | Behavior Spec |
| Stage 3 (기술 명세) | 파일 수준 변경 맵 확정 | Stage 3.5 또는 Stage 4 | Technical Spec |
| Stage 3.5 (Codex 플래닝) | Codex 플랜 수집/병합 | Stage 4 | `CODEX_PLAN` (선택) |
| Stage 4 (체크리스트) | 구현 체크리스트 확정 | 합의 검토 | `plan.md` draft |
| 합의 검토 | 합의 도달 | 사용자 검토/최종화 | 검토 요약 |
| 사용자 검토/최종화 | 승인 + 워크로그 업데이트 완료 | 종료 | `plan.md` |

오케스트레이션 전이 계약:
- `ORCHESTRATED=true` + `current_phase=PRD` → Stage 1만 실행 후 종료
- `ORCHESTRATED=true` + `current_phase=PLAN` → Stage 1 건너뛰고 Stages 2-4 실행
...
3. 기준 브랜치 결정: `resolve-base-branch` 스킬을 로드하고 해당 로직을 따라 `BASE_REF`를 설정한다.
...
플랜 완료 시 아래 순서를 고정한다:
1. `prd.md`/`plan.md` 작성 또는 갱신
2. 워크로그 업데이트 (`_shared/update-worklog.md`)
3. 사용자에게 출력/인계 메시지 제공

## 절대 규칙

- **무거운 작업은 위임한다** — `_shared/delegation-policy.md` 참조
- **사용자 명시 요청 없이는 단계 생략/워크플로우 변경 금지.** vplan이 임의로 Stage를 건너뛰거나 순서를 재배치하지 않는다.
- **정규 단계 테이블(SSOT)을 따른다.** 오케스트레이션 전이 계약 외 예외 전이는 허용하지 않는다.
