---
name: vbrowser
description: Playwright MCP로 PRD 유저 시나리오와 테스트 케이스를 브라우저에서 직접 실행하는 E2E 테스트 스킬. vtest에서 호출되거나 단독 실행 가능.
...
- 그 외: `_shared/resolve-worklog-target.md`를 로드하고 해당 절차를 따른다.
...
### Step 1: dev server URL 감지

`browser_tabs`로 현재 Playwright 브라우저에 열린 탭 목록을 확인한다.
...
Step 1에서 URL을 찾지 못한 경우, `{WORKLOG_DIR}/worklog.md`를 읽어 다음 필드를 탐색한다:
...
각 시나리오의 주요 단계마다 스크린샷이 캡처됩니다. 실패 지점에서 스크린샷이 자동 저장됩니다.
```
...
FOR each scenario in selected_scenarios:
  ...
  SCREENSHOT_DIR = WORKLOG_DIR != null
    ? "{WORKLOG_DIR}/screenshots/{SCENARIO_IDX}-{SCENARIO_SLUG}/"
    : "/tmp/vbrowser-screenshots/{SCENARIO_IDX}-{SCENARIO_SLUG}/"
  RESULT = "PASS"
  FAIL_REASON = null
  ...
  # 시작점으로 이동
  browser_navigate(url=scenario.start_url or APP_URL)

  # 초기 상태 스크린샷
  browser_take_screenshot() → 저장 경로 STEP_SCREENSHOTS에 추가
  ...
  FOR each step in scenario.steps:
    ...
    CATCH failure (도구 오류 / 예상 요소 없음 / 예상 상태 불일치):
      ...
      # 실패 지점 스크린샷
      browser_take_screenshot() → FAIL_SCREENSHOT
...
| 단계 유형 | 사용 도구 | 비고 |
|----------|----------|------|
| 페이지 이동 | `browser_navigate` | 상대 경로는 `APP_URL` 기준으로 절대 경로 조합 |
| 텍스트 입력 | `browser_snapshot` → `browser_type(element, ref, text)` | 접근성 트리에서 ref 탐색 후 입력 |
| 버튼/링크 클릭 | `browser_snapshot` → `browser_click(ref)` | ref 기반 클릭 |
| 체크박스/라디오/셀렉트 | `browser_snapshot` → `browser_select_option(ref, values)` | |
| 페이지 텍스트 확인 | `browser_snapshot` | 접근성 트리에서 예상 문자열 포함 여부 확인 |
| 시각적 요소 확인 | `browser_take_screenshot` | 텍스트 기반 확인 불가 시 |
| JS 직접 실행 | `browser_evaluate` | DOM 조작, 이벤트 트리거 |
| 콘솔 에러 확인 | `browser_console_messages` | 실패 원인 추가 정보 수집 시 |
| 대기 | `browser_wait_for(text)` 또는 `browser_wait_for(time)` | 동적 로딩 대기 |
| 폼 일괄 입력 | `browser_fill_form(fields)` | 여러 필드 한 번에 입력 |

### 실패 판단 기준

다음 중 하나라도 해당하면 FAIL 처리:
- 예상 DOM 요소가 `browser_snapshot` 접근성 트리에서 ref로 발견되지 않음
- 예상 텍스트/메시지가 페이지에 없음
...
- **다이얼로그 자동 dismiss**: 예상치 못한 다이얼로그 발생 시 `browser_handle_dialog(accept=false)`로 dismiss 처리한다. 가급적 다이얼로그를 유발하는 요소(삭제 버튼 등)는 클릭하지 않으며, 불가피한 경우 dismiss 후 SKIP으로 기록한다.
- **격리된 브라우저**: Playwright MCP는 별도 브라우저 인스턴스를 사용하므로 사용자 브라우저에 영향이 없다.
...
| # | 시나리오 | 결과 | 증거 | 실패 원인 |
|---|---------|------|------|----------|
| 1 | {name} | ✅ PASS | {SCREENSHOT_DIR} | - |
| 2 | {name} | ❌ FAIL | {FAIL_SCREENSHOT} | {FAIL_REASON} |
...
- `stable_updates`:
  - Links: 스크린샷 디렉토리 경로 목록, 보고서 경로 추가
...
## UI 패턴 / 접근성 ref
<!-- 자주 쓰이는 접근성 ref, 특이한 UI 구조, 동적 요소 패턴 등 -->
...
**`ORCHESTRATED=true`인 경우:** 브라우저 테스트 완료 후 바로 종료. vwork가 결과를 워크로그에서 읽어 다음 phase 전이를 관리한다.
...
- **스크린샷 증거 필수.** 모든 시나리오의 각 주요 단계에서 `browser_take_screenshot`으로 캡처한다.
- **실패 시 스크린샷 필수.** FAIL 지점에서 반드시 `browser_take_screenshot`으로 캡처한다.
- **다이얼로그 자동 dismiss.** `browser_handle_dialog(accept=false)`로 처리. 예상치 못한 다이얼로그 발생 시 SKIP 처리.
- **격리된 브라우저.** Playwright MCP는 별도 브라우저 인스턴스를 사용하므로 사용자 브라우저에 영향 없음.
