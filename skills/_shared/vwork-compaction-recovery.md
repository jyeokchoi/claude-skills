...
- `codex_available` = `state.codex_available ?? false`
- `gemini_available` = `state.gemini_available ?? false`

**행동 규칙 복구 (compaction 후 규칙 유실 방지):**
- `state.mode`를 기준으로 복구 대상 스킬 파일을 결정한다:
  - `mode == "auto"` → `Glob(pattern="**/workflow/vauto/SKILL.md")`
  - 그 외 → `Glob(pattern="**/workflow/vwork/SKILL.md")`
...
`cli_workers` 맵을 복구한다: `cli_workers` = `state.cli_workers` (없으면 빈 맵 `{}`)

현재 phase의 필수 팀원 목록을 확인한다 (SKILL.md 정규 매핑 테이블 참조):

각 필수 팀원에 대해 **팀원 유형 선행 분기**:

**CLI 워커 팀원** (`cli_workers[{팀원명}]`이 존재하는 경우):
- config.json member check **건너뜀** (CLI 워커는 config.json에 등록되지 않음)
- 조회 대상 작업 ID: `CLI_JOB_ID = cli_workers[{팀원명}].job_id` (없으면 상태 불일치로 간주하고 해당 항목 제거)
- `omc_run_team_status`로 직접 확인:
  ```
  ToolSearch(query="+omc_run_team_status")
  mcp__plugin_oh-my-claudecode_team__omc_run_team_status({"job_id": "{CLI_JOB_ID}"})
  ```
  - 결과 `running`: 활성 → 유지
  - 결과 `completed` / `failed` / `not_found`: 비활성 → claude Task() fallback으로 전환 (재스폰 않음), `cli_workers`에서 해당 항목 제거, `state_write`로 갱신

**Claude 에이전트 팀원** (`cli_workers[{팀원명}]`이 없는 경우):
1. `~/.claude/teams/{team_name}/config.json`이 **없으면**: 팀 종료됨 → **TeamDelete → Step 3-B로 점프**
2. `config.json`이 **존재하면**:
   - config.json의 `members` 배열에서 해당 팀원 확인
   - 팀원이 없으면: **TeamDelete → Step 3-B로 점프**
   - 팀원이 있으면: ping 전송:
     ```
...
**전원 확인 완료 후 (모든 팀원이 활성 상태):** → **Step 4 (Phase 실행)로 직접 점프**
