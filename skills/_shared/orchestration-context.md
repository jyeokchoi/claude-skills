...
ORCHESTRATION_MODE = ORCHESTRATED ? (vwork_state.mode == "session" ? "auto" : vwork_state.mode) : null
# 주: session 모드는 서브 스킬 동작상 auto와 동일하므로 "auto"로 정규화한다.
```
...
> **session 모드**: vwork의 `mode`가 `"session"`인 경우, 위 읽기 프로토콜에서 `"auto"`로 정규화된다. 따라서 서브 스킬 입장에서 오케스트레이션 환경은 auto와 동일하다. (session 모드에서 vwork 오케스트레이터는 SendMessage로 팀원에게 작업을 위임하고, 팀원이 서브 스킬을 호출하는 구조이므로 이 정규화가 올바르다.)
