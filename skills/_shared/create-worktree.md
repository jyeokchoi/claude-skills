...
- `base_ref`: 분기 기준 (e.g., `develop`, `upstream/develop`, `upstream/{branch}`)
...
```bash
git worktree add {worktree_path} upstream/{branch_name}
cd {worktree_path} && git checkout -B {branch_name} upstream/{branch_name}
```
...
**제공되지 않은 경우:** `rules/project-params.local.md`에서 `dependency_install` 설정을 읽는다.

**설정도 없는 경우:** 프로젝트에 맞는 패키지 매니저를 자동 탐지:

```bash
# 1. rules/project-params.local.md의 dependency_install 사용 (있으면)
