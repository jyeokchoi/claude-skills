...
|------|----------------------|---------------------|------------------------|
| `cli_type=codex` | codex (omc-teams MCP) | Claude (quality-reviewer, sonnet) | codex (omc-teams MCP) |
...
```
mcp__team__omc_run_team_start(count=1, provider="codex", task="구조 분석: {Agent 1 프롬프트}. 테스트 분석: {Agent 3 프롬프트}")
```
