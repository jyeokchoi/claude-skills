...
프로젝트 루트 `rules/project-params.local.md`가 존재하면:
- `jira_pattern` 설정에서 프로젝트 키 prefix 추출 (e.g., `VREW-\d+` → `VREW`)

`rules/project-params.local.md`가 없거나 `jira_pattern`이 설정되지 않은 경우:
...
- `issue_url`: 다음 우선순위로 결정
  1. 프로젝트 루트 `rules/project-params.local.md`의 `jira_base_url` 설정이 있으면 `{jira_base_url}{issue_key}` 사용
