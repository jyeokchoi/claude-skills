...
- 기존 `:cli:` 파싱과의 호환성: `spawned_agents` 항목을 파싱할 때 `:` 기준 앞부분(이름)을 역할 식별에 사용하고, `:cli:` 접미사는 팀원 유형 식별에만 사용한다.
...
`spawned_agents` 항목에서 `:cli:` 접미사 여부로 유형을 식별한다.
...
**CLI 워커 팀원 (`{name}:cli:{codex|gemini}`):**
```
ToolSearch(query="+omc_run_team_cleanup")
mcp__plugin_oh-my-claudecode_team__omc_run_team_cleanup({"teamName": "{team_name}"})
```
완료 후 `spawned_agents`에서 해당 항목(`:cli:` 접미사 포함) 제거.

정리 완료 후 상태 보존 규칙에 따라 `state_write(mode="vwork")`로 `spawned_agents` 갱신.
...
스폰 후 `spawned_agents`에 `{name}:cli:{codex|gemini}` 형식으로 기록한다.
